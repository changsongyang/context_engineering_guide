...
|------|--------|------------|------------|
| OpenAI GPT 系列 | 当前快照中官方目录首位已是 GPT-6 Astra（滚动开放中，非全量可用）；已全量可用的档位以 GPT-5.6 Sol 作为复杂推理和编码旗舰，GPT-5.6 Terra 平衡智能与成本，GPT-5.6 Luna 面向成本敏感的高吞吐负载；GPT-5.3-Codex 面向编码智能体 | 通用推理、代码与工具生态成熟 | 按 Sol、Terra、Luna 的能力与成本边界分层评估；价格和可用性按模型 ID、账号层级、缓存、区域与服务层级核验 |
